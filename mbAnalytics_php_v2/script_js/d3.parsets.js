...
(function() {
  d3.parsets = function() {
    var event = d3.dispatch("sortDimensions", "sortCategories"),
     ...
     function parsets(selection) {
      selection.each(function(data, i) {
        ...
        g.selectAll(".ribbon-parsets, .ribbon-mouse-parsets")
            .data(["ribbon-parsets", "ribbon-mouse-parsets"], String)
          .enter().append("g")
            .attr("class", String);
        updateDimensions();
        if (tension != tension0) {
        ...
        function updateDimensions() {
          ...
          var dimension = g.selectAll("g.dimension-parsets"),
              cache = {};
          ...
          g.select(".ribbon-parsets").selectAll("path")
              .each(function(d) {
          ...
          textEnter.append("tspan")
              .attr("class", "name-parsets")
              .text(dimensionFormatName);
          /* MAX: funzioni di ordinamento commentate
          textEnter.append("tspan")
              .attr("class", "sort-parsets alpha")
              .attr("dx", "2em")
              .text("alpha »")
              .on("mousedown.parsets", cancelEvent);
          textEnter.append("tspan")
              .attr("class", "sort-parsets size")
              .attr("dx", "2em")
              .text("size »")
              .on("mousedown.parsets", cancelEvent);*/
          dimension
                ...
                .on("drag", function(d) {
                  ...
                  for (var i = 1; i < dimensions.length; i++) {
                    if (height * dimensions[i].y < height * dimensions[i - 1].y) {
                      ...
                      total = getTotal(dimensions);
                      g.selectAll(".ribbon-parsets, .ribbon-mouse-parsets").selectAll("path").remove();
                      updateRibbons();
          ...
          dimension.select("text").select("tspan.sort-parsets.alpha")
              .on("click.parsets", sortBy("alpha", function(a, b) { return a.name < b.name ? 1 : -1; }, dimension));
          dimension.select("text").select("tspan.sort-parsets.size")
          ...
          // MAX: funzione per ordinare le dimensioni
          sortByMax(dimension);
        }
        
        // BEGIN MAX
        function sortByMax(dimension) {
          dimensions.forEach(function(d, i) {
            	d.categories.sort(function(a, b) {
            		var resA = a.name.split(" - ");
            		var resB = b.name.split(" - ");
            		
            		var minIntA = parseFloat(resA[0]);
            		var minIntB = parseFloat(resB[0]);
            		return minIntA < minIntB ? -1 : 1;
            	});
          });
          nodes = layout(tree, dimensions, ordinal);
          updateCategories(dimension);
        ...
        function updateRibbons() {
          ribbon = g.select(".ribbon-parsets").selectAll("path")
          ...
          ribbon.enter().append("path")
              ...
              .attr("class", function(d) { return "category-parsets-" + d.major; })
              .attr("d", ribbonPath);
          ribbon.sort(function(a, b) { return b.count - a.count; });
          ribbon.exit().remove();
          var mouse = g.select(".ribbon-mouse-parsets").selectAll("path")
              .data(nodes, function(d) { return d.path; });
          mouse.enter().append("path")
              .on("mousemove.parsets", function(d) {
                ribbon.classed("active-parsets", false);
                if (dragging) return;
        ...
        function updateCategories(g) {
          ...
          category.select("rect")
              ...
              .attr("class", function(d) {
                return "category-parsets-" + (d.dimension === dimensions[0] ? ordinal(d.name) : "background");
              });
    ...
    var tooltip = body.append("div")
        .style("display", "none")
        .attr("class", "parsets tooltip-parsets");
